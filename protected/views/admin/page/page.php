    ...
    <?php if(!empty($arguments['result'])) : ?>    
        <?php foreach ($arguments['result'] as $key => $value) : ?>
            <div class="admin-page block-padding">
                <div class="page-title bold-title">
                    <?=$value['title'];?>
                </div>
                <div class='btn-add-page'>
                    <a href='/admin/addPage/<?=$value['id_page_type']?>' class='btn btn-green'>Добавить материал</a>  
                </div>
            </div>
            <div class='admin-inner-page-edit'>
                ...
                <?php foreach($value['page'] as $key2 => $value2) : ?>
                    <div class='admin-edit-row'>
                        ...
                        <div class='btn-add-material'>
                            <a href='/admin/editPage/<?=$value2['id_page']?>/<?=$value['id_page_type']?>' class='btn btn-blue'>Редактировать</a> 
                        </div>
                    </div>
                <?php endforeach;?>
                <?php endif;?>
