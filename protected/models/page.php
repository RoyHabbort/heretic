...
class page extends modelsClass{
    ...
    public function getTopMenu(){
        $sql = "SELECT * FROM `page` p
                WHERE p.parent_id = 0 ORDER BY `sort` ASC";
        $result = $this->query($sql);
        
        foreach ($result as $key => $value) {
            $sql = "SELECT * FROM `page` WHERE `parent_id` = '{$value['id_page']}' ORDER BY `page_type` ASC, `sort` ASC";
            $result[$key]['child'] = $this->query($sql);
        }
