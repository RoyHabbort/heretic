...
class admin extends modelsClass {
    ...
    public function addField($params){
        $params = $this->validate($params, $rules = array(
                ...
                'string' => 'title, field_name, field_type_id, image_filter'
                ));
        ...
        if($params['field_type_id'] != 6){
            $params['image_filter'] = '';
        }
        
        $sql = "INSERT INTO `field` (`title`, `field_name`, `field_type_id`, `image_filter`) VALUE ('{$params['title']}', '{$params['field_name']}','{$params['field_type_id']}', '{$params['image_filter']}' )";
        $result = $this->query($sql);
        if(!$result){
            return $this->bdError;
        }
    ...
    public function getFieldValue($idPage, $idPageType = 0){
        ...
        if($auxField){
            ...
            foreach ($auxField as $key => $value) {
                ...
                if($condit){
                    $sql = "SELECT `field_content_{$value['field_name']}`, `id_field_{$value['field_name']}` FROM `field_inner_{$value['field_name']}` WHERE `page_id_{$value['field_name']}` = '{$params['id_page']}' ORDER BY `page_id_{$value['field_name']}` ASC";
